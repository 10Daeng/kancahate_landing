...
export const bigFiveQuestions = questions

export const bigFiveChoices = [
  { text: 'Sangat Tidak Akurat', value: 1, color: 'bg-red-50 border-red-200 text-red-700' },
  { text: 'Tidak Akurat', value: 2, color: 'bg-orange-50 border-orange-200 text-orange-700' },
  { text: 'Netral', value: 3, color: 'bg-slate-50 border-slate-200 text-slate-700' },
  { text: 'Akurat', value: 4, color: 'bg-green-50 border-green-200 text-green-700' },
  { text: 'Sangat Akurat', value: 5, color: 'bg-emerald-50 border-emerald-200 text-emerald-700' }
];

export const calculateBigFiveResults = (answers) => {
  const scores = { N: 0, E: 0, O: 0, A: 0, C: 0 };
  const counts = { N: 0, E: 0, O: 0, A: 0, C: 0 };
  
  // Create a map for O(1) lookup
  const questionMap = new Map(bigFiveQuestions.map(q => [q.id, q]));
...
export const DOMAIN_LABELS = {
  O: { title: 'Openness (Keterbukaan)', desc: 'Imajinatif, penasaran, dan berani mencoba hal baru.' },
  C: { title: 'Conscientiousness (Kesadaran)', desc: 'Terorganisir, dapat diandalkan, dan disiplin.' },
  E: { title: 'Extraversion (Ekstraversi)', desc: 'Sosial, energik, dan percaya diri.' },
  A: { title: 'Agreeableness (Keramahan)', desc: 'Kooperatif, sopan, dan peduli pada orang lain.' },
  N: { title: 'Neuroticism (Neurotisme)', desc: 'Kecenderungan merasakan emosi negatif seperti cemas/sedih.' }
};
