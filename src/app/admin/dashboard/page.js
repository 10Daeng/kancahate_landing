...
export default function AdminDashboardPage() {
  ...
  const fetchUserStats = async () => {
    try {
      // Get user count from auth.users (via view)
  ...
  const fetchArticleStats = async () => {
    try {
      ...
      return {
        ...
        totalArticleViews: articles.reduce((sum, a) => sum + (a.view_count || 0), 0),
      };
    } catch (error) {
  ...
  const fetchTestStats = async () => {
    try {
      // Get test results from all test tables
      const tables = [
        { name: 'riasec_results', key: 'riasecTests' },
        { name: 'mbti_results', key: 'mbtiTests' },
        { name: 'bigfive_results', key: 'bigFiveTests' },
        { name: 'vark_results', key: 'varkTests' },
        { name: 'love_language_results', key: 'loveLanguageTests' },
        { name: 'mi_results', key: 'miTests' },
        { name: 'rimb_results', key: 'rimbTests' },
      ];
      ...
      for (const table of tables) {
        try {
          ...
          if (!error && data) {
            testCounts[table.key] = data.length;
            totalTests += data.length;
            data.forEach(d => {
              if (d.user_id) uniqueUsers.add(d.user_id);
            });
          } else {
            testCounts[table.key] = 0;
          }
        } catch (e) {
          testCounts[table.key] = 0;
        }
  ...
  const fetchDailyActivity = async () => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        // Article views (placeholder - would need analytics table)
        dailyViews.push(Math.floor(Math.random() * 100) + 20);

        // Tests taken that day
        let testsDay = 0;
        const testTables = ['riasec_results', 'mbti_results', 'bigfive_results', 'vark_results', 'love_language_results', 'mi_results', 'rimb_results'];
        for (const table of testTables) {
          try {
            ...
            if (data) testsDay += data.length;
          } catch (e) {
            // Ignore
          }
        }
        dailyTests.push(testsDay);
  ...
  const fetchTopContent = async () => {
    try {
      ...
      // Top tests
      const testStats = [
        { name: 'RIASEC', icon: '🎯', count: stats.riasecTests || 0 },
        { name: 'MBTI', icon: '🧠', count: stats.mbtiTests || 0 },
        { name: 'Big Five', icon: '📊', count: stats.bigFiveTests || 0 },
        { name: 'VARK', icon: '👁️', count: stats.varkTests || 0 },
        { name: 'Love Language', icon: '❤️', count: stats.loveLanguageTests || 0 },
        { name: 'Multiple Intelligence', icon: '💡', count: stats.miTests || 0 },
        { name: 'RIMB', icon: '👥', count: stats.rimbTests || 0 },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-bold text-slate-800">Dashboard Statistik</h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={fetchStats}
                disabled={refreshing}
                className="flex items-center gap-2 px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-xl font-bold text-sm transition-colors disabled:opacity-50"
              >
