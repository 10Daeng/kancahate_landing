...
// Deteksi dengan regex untuk menangkap teks yang disamarkan (mis: "b.u.n.u.h")
...
export function detectCrisisLevel(text) {
  ...
  // 1. Cek Regex Kritis (Langsung Kritis / Skor +5)
  for (const pattern of criticalPatterns) {
    if (pattern.test(normalizedText)) {
      totalScore += 5;
      detectedKeywords.push('suicide/self-harm pattern');
      break; // Sudah cukup memicu Kritis
    }
  }
  ...
  // 4. Cek Kata Kunci Rendah (hanya per kata independen)
